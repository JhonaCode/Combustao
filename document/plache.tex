The bidimensional steady compressible boundary layer equation for with zero
streamwise pressure gradient and non-body forces for a one step chemical
reaction base on \cite{planche1992numerical} can be writem for  conservation of mass as   


\begin{equation} 
\frac{\partial \rho^* u^* }{\partial x^*}+
\frac{\partial \rho^* v^* }{\partial y^*}+
=0
\end{equation}

Considering a single step chemical reaction,

\begin{equation}
\nu_F F + \nu_O O \rightarrow \nu_P P
\end{equation}

where $\nu_F$ and $\nu_O$
are the stiochiometric coeficients, for the 
fuel and oxidizer, respectively. 
Considering $\nu_F=1$ and $\nu_O=n$, indicating that 
$n$ moles of oxidizer are
required to burn one mole of fuel, producing $\nu_p=(1+n)$ moles of product,

\begin{equation}
F + n O \rightarrow (1+n)P
\end{equation}

where $n$ is the stiochiometric 
mass ration between the fuel and oxidizer, which can be expressed as

\begin{equation}
n=\frac{W_F}{W_O}\frac{\nu_O}{\nu_F}
.
\end{equation}

Where $W_F$ and $W_O$ are the molecular 
weights of the fuel and oxidizer, respectively.

Then the oxidizer mass consevation equation is 

\begin{equation} 
\rho^* u^*\frac{\partial Y_{O}^*}{\partial x^*}+
\rho^* v^*\frac{\partial Y_{O}^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left(\rho^* D_{O}^{*}\frac{\partial Y_O^{*}}{\partial y^{*}}\right)
-
\dot{\omega}_O
%\frac{\partial\rho D^m \tfrac{\partial\rho Y^m u_i}{\partial x_i}u_i}{\partial x_i}=
,
\end{equation} 

the fuel mass conservation

\begin{equation} 
\rho^* u^*\frac{\partial Y_{F}^*}{\partial x^*}+
\rho^* v^*\frac{\partial Y_{F}^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left(\rho^* D_{F}^{*}\frac{\partial Y_F^{*}}{\partial y^{*}}\right)
-
\dot{\omega}_F
%\frac{\partial\rho D^m \tfrac{\partial\rho Y^m u_i}{\partial x_i}u_i}{\partial x_i}=
\end{equation} 

and the product conservation 

\begin{equation} 
\rho^* u^*\frac{\partial Y_{P}^*}{\partial x^*}+
\rho^* v^*\frac{\partial Y_{P}^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left(\rho^* D_{P}^{*}\frac{\partial Y_P^{*}}{\partial y^{*}}\right)
+
\dot{\omega}_p
.
\end{equation} 

All species production rates depend on one global reaction rate

\begin{equation}
    \dot{\omega}_i=W_i n \dot{\Omega}
\end{equation}

Where, for the species mass conservation in the chemical reaction  

\begin{equation}
\sum_i\dot{\omega}_i=0
\end{equation}

The species equation are expressed in terms of viscosity 
using  the fact:

\begin{equation}
    \rho^*D_i^{*}
    =
    \rho^*\alpha^{*}\frac{D_i^{*}}{\alpha^{*}}
    =
    \rho^*\frac{c_p^*\rho^*}{k^*}\frac{1}{Le_i}
    =
    \mu^*\frac{c_p^*\mu^*}{k^*}\frac{1}{Le_i}
    =
    \mu^*
    \frac{1}{Le_i}
    \frac{1}{Pr}
\end{equation}

The momemtum streamwise equation 

\begin{equation} 
\rho u^*\frac{\partial u^*}{\partial x^*}+
\rho v^*\frac{\partial u^*}{\partial y^*}=-
\frac{\partial}{\partial y^*}
\left(\mu^*\frac{\partial u^*}{\partial y^*}\right)
.
\end{equation} 

The entalphy $h=e+p/\rho$ form of the energy equation:

\begin{equation}
%\frac{\partial}{\partial t}(\rho h)+
\rho^* u^* \frac{\partial h^*}{\partial x^*}+
\rho^* v^* \frac{\partial h^*}{\partial y^*}
=
\mu^*
\left(
\frac{\partial u^*}{\partial y^*}
\right)^2
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}
        \rho 
        D_i
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{equation} 


where $h_{si}$ is the sensible enthalpy 
for the specie $i$ in a multicomponent misture.  
For multiples species $h_s=\sum Y_ih_{si}(T)$
with  

\begin{equation} 
h_{si}^*
=
\int_{T_0^*}^{T^*}
c_{pi}^*dT^*
\end{equation} 

and the formation enthalpy of the misture  
can be defined 

\begin{equation} 
   h_{f}^\circ
   =
   \sum
   h_{fi}^{\circ*}
   .
\end{equation} 

Enthalpy can divide explicitly in two parts 

\begin{equation} 
    h^*=h_s^*+h_f^{\circ*}
\end{equation} 

for multiple species 

\begin{equation} 
    h^*=
    \sum_i
    \int_{T_0^*}^{T^*}
    c_{pi}^*dT^*
    +
    \sum_i h_{fi}^{*\circ}
\end{equation} 


Taking the material derivative

\begin{equation} 
\begin{split}
    \frac{D h^*}{D t^*}
    =
    \frac{D h_s^*}{D t^*}
    +
    \frac{D h_f^{\circ *}}{D t^*}
    =
    \frac{D }{D t^*}
    \sum_i Y_i^* h_{si}^*(T^*)
    +
    \frac{D }{D t^*}
    \sum_i Y_i^* h_{fi}^{\circ*}
    =
    \\
    \sum_i  
    \frac{D Y_i^*}{D t^*}
    h_{si}^*(T^*)
    +
    \sum_i  
    \frac{D h_{si}^*(T^*)}{D t^*}
    Y_i
    +
    \sum_i  
    \frac{D Y_i^*}{D t^*}
    h_{fi}^{\circ*}
    +
    \cancelto{0}{
    \sum_i  
    \frac{D h_{fi}^{\circ*}}{D t^*}
    Y_i
    }
    =
    \\
    \frac{D }{D t^*}
    \sum_i Y_i^* h_{si}^*(T^*)
    +
    \sum_i  
    \frac{D Y_i^*}{D t^*}
    h_{fi}^{\circ*}
\end{split} 
\end{equation} 

In this case special case $\dfrac{D}{D t^*}=u^*\dfrac{\partial }{\partial x^*}+v^*\dfrac{\partial }{\partial y^*}$ and 
the last term vanish once the formation enthalpy is constant for each specie. 
The term $\dfrac{D Y_i^*}{D t^*}$ is the formation rate of the specie $i$, normally 
write as $\dot{\omega}_i^*$.

%The species equation can multiply by $h_{si}$ and  summed: 
%
%\begin{equation}
%    \rho^*
%    \sum_i  
%    h_{si}^*
%    \frac{D Y_i^*}{D t^*}
%    =
%    \sum_i  
%    \frac{\partial }{\partial y^*}
%    \left(
%        \rho^*D_i^*
%        \frac{\partial Y_i^* }{\partial y^*}
%    \right)
%\end{equation}

So, with the above sepation the  energy enthalpy equation gets :

\begin{equation}
\frac{D h_{s}^*(T^*)}{D t^*}
=
\mu^*
\left(
\frac{\partial u^*}{\partial y^*}
\right)^2
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}
        \rho 
        D_i
        \frac{\partial Y_i^*}{\partial y^*}
\right)
-
\sum_i  
\dot{\omega}_i
h_{fi}^{\circ*}
\end{equation} 

The heat realease of the chemical reaction  
can be writen as: 

\begin{equation} 
    Q_{rxn}
    \dot{\omega}
    =
    \sum_i h_{fi}^{*\circ}
    \dot{\omega}_i
\end{equation} 

where the formation enthalphy  $h_{fi}^{*\circ}$ 
of the specie $i$ is
the enthalphy change when 1 mole of a compound 
is formed from its elements in their standart state. 

For the boundary layer equation the energy equation 
for the reacting misture is  

\begin{equation}
\rho^* u^* \frac{\partial h_s^*}{\partial x^*}+
\rho^* v^* \frac{\partial h_s^*}{\partial y^*}
=
\mu^*\frac{\partial^2 u^*}{\partial (y^*)^2}+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
-
\sum_i  
\dot{\omega}_i
h_{fi}^{\circ*}
\end{equation} 

% Reaction rate sign convention
\begin{equation} 
\dot{\omega}_i^* > 0 \quad \text{means species } i \text{ is produced}
\end{equation} 

Products of an exothermic reaction usually have lower formation enthalpy

% Standard reaction enthalpy
\begin{equation} 
\Delta H^{\circ*}_{\text{reaction}}
=
\sum_{\text{products}} h_{fi}^{\circ*}
-
\sum_{\text{reactants}} h_{fi}^{\circ*}
< 0
\quad \text{(exothermic reaction)}
\end{equation} 

then the term: 

\begin{equation} 
% Source term in sensible enthalpy equation
\sum_i h_{fi}^{\circ}\dot{\omega}_i < 0
\end{equation} 

if reaction is exothermic. Therefore in the enthalply equation :

\begin{equation} 
- \left( \sum_i h_{fi}^{\circ}\dot{\omega}_i \right) > 0
\end{equation} 

Meaning that positive heat release term
\begin{equation} 
\text{Exothermic reaction} \;\Rightarrow\;
\text{positive contribution to the energy equation}
.
\end{equation} 

A equation for total enthapy can be 
obtained using its definition 

\begin{equation} 
    H^{*}=h_s^{*}+h_f^{\circ*}+\frac{1}{2}u^{*2}
\end{equation} 

Multiply the momemtum equation by $u^{*}$ 
and adding to the energy entalphy equation 

\begin{equation}
\begin{split}
\rho^* u^* \frac{\partial h_s^*}{\partial x^*}
+
u^*
\rho u^*\frac{\partial u^*}{\partial x^*}
+
\rho^* v^* \frac{\partial h_s^*}{\partial y^*}
+
u^*
\rho v^*\frac{\partial u^*}{\partial y^*}
=
\\
\mu^*
\left(
\frac{\partial u^*}{\partial y^*}
\right)^2
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
-
\sum_i  
\dot{\omega}_i
h_{fi}^{\circ*}
+
u^*
\frac{\partial}{\partial y^*}
\left(\mu^*\frac{\partial u^*}{\partial y^*}\right)
\end{split}
\end{equation} 


\begin{equation}
\begin{split}
\rho^* u^* \frac{\partial h_s^*}{\partial x^*}
+
u^*
\rho u^*\frac{\partial u^*}{\partial x^*}
+
\rho^* v^* \frac{\partial h_s^*}{\partial y^*}
+
u^*
\rho v^*\frac{\partial u^*}{\partial y^*}
=
\\
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
-
\cancel{
\mu
\frac{\partial u^*}{\partial y^*}
\frac{\partial u^*}{\partial y^*}
}
+
\cancel{
\mu^*
\left(
\frac{\partial u^*}{\partial y^*}
\right)^2
}
+
\\
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
-
\sum_i  
\dot{\omega}_i
h_{fi}^{\circ*}
\end{split}
\end{equation} 

The species equation multiply by $h_{fi}^{\circ}$ and summed 
for all species 

\begin{equation}
    \rho^*
    \sum_i  
    h_{fi}^{\circ*}
    \frac{D Y_i^*}{D t^*}
    =
    \sum_i  
    h_{fi}^{\circ*}
    \frac{\partial }{\partial y^*}
    \left(
        \rho^*D_i^*
        \frac{\partial Y_i^* }{\partial y^*}
    \right)
    +
    \sum_i
    h_{fi}^{\circ*}
    \dot{\omega}_i^{*}
\end{equation}

\begin{equation}
    \rho^*
    \sum_i  
    \frac{D }{D t^*}
    \left(
        h_{fi}^{\circ*}
        Y_i^*
    \right)
    - 
    \cancel{
    \rho^*
    \sum_i  
    Y_i^*
    \frac{D h_{fi}^{\circ*}}{D t^*}
    }
    =
    \sum_i  
    h_{fi}^{\circ*}
    \frac{\partial }{\partial y^*}
    \left(
        \rho^*D_i^*
        \frac{\partial Y_i^* }{\partial y^*}
    \right)
    +
    \sum_i
    h_{fi}^{\circ*}
    \dot{\omega}_i^{*}
\end{equation}

is added 

\begin{equation}
\begin{split}
\rho^* u^* \frac{\partial h_s^*}{\partial x^*}
+
u^*
\rho u^*\frac{\partial u^*}{\partial x^*}
+
\rho^* v^* \frac{\partial h_s^*}{\partial y^*}
+
u^*
\rho v^*\frac{\partial u^*}{\partial y^*}
+
\rho^*
\sum_i  
\frac{D }{D t^*}
\left(
    h_{fi}^{\circ*}
    Y_i^*
\right)
=
\\
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
-
\cancel{
\sum_i  
\dot{\omega}_i
h_{fi}^{\circ*}
}
\\
+
\sum_i  
h_{fi}^{\circ*}
\frac{\partial }{\partial y^*}
\left(
    \rho^*D_i^*
    \frac{\partial Y_i^* }{\partial y^*}
\right)
+
\cancel{
\sum_i
h_{fi}^{\circ*}
\dot{\omega}_i^{*}
}
\end{split}
\end{equation} 

\begin{equation}
\begin{split}
\rho^* u^* \frac{\partial h_s^*}{\partial x^*}
+
u^*
\rho u^*\frac{\partial u^*}{\partial x^*}
+
\rho^* v^* \frac{\partial h_s^*}{\partial y^*}
+
u^*
\rho v^*\frac{\partial u^*}{\partial y^*}
+
\rho^*
\frac{D h_{f}^{\circ*}}{D t^*}
=
\\
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\\
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
+
\sum_i  
h_{fi}^{\circ*}
\frac{\partial }{\partial y^*}
\left(
    \rho^*D_i^*
    \frac{\partial Y_i^* }{\partial y^*}
\right)
\end{split}
\end{equation} 


\begin{equation}
\begin{split}
\rho^* u^* \frac{\partial H^*}{\partial x^*}
+
\rho^* v^* \frac{\partial H^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
\\
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
+
\sum_i  
h_{fi}^{\circ*}
\frac{\partial }{\partial y^*}
\left(
    \rho^*D_i^*
    \frac{\partial Y_i^* }{\partial y^*}
\right)
\end{split}
\end{equation} 

Therefore, the total enthalpy equation form 
of the energy equation is given by :

\begin{equation}
\begin{split}
\rho^* u^* \frac{\partial H^*}{\partial x^*}
+
\rho^* v^* \frac{\partial H^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
\\
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{i}(T^*)
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split}
\end{equation} 

{\color{red}
Now a special form of the total enthalpy equation can 
be obtained assuming that the specific heat at 
constant pressure is constant 

\begin{equation} 
    h_s^*
    =
    \int_{T_0^*}^{T^*}
    c_p^*(T^*)
    dT^*
    =
    c_p^*
    T^*
\end{equation} 
}
it is no necessary, we need the derivative:

\begin{equation} 
    \frac{\partial h_s^*}{\partial T}
    =
    \frac{\partial }{\partial T}
    \int_{T_0^*}^{T^*}
    c_p^*(T^*)
    dT^*
    =
    c_p^*
\end{equation} 

\begin{equation} 
    \frac{\partial T^*}{\partial y^*}
    =
    \frac{\partial T^*}{\partial h_s^*}
    \frac{\partial h_s^*}{\partial y^*}
    =
    \frac{1}{c_p}
    \frac{\partial h_s^*}{\partial y^*}
\end{equation} 

and the thermal diffusion and viscous work can be 
unified  

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 


\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    k^*\frac{\partial T^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left(
    \frac{k^*}{c_p^*}
    \frac{\partial h_s^*}{\partial y^*}
\right)
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left[
    \frac{k^*}{c_p^*}
    \left(
        \sum_i
        \frac{\partial Y_i}{\partial y^*}
        h_{si}
        +
        \sum_i
        Y_i
        \frac{\partial h_{si} }{\partial y^*}
    \right)
\right]
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 

The derivate of the Total enthalpy is 

\begin{equation*} 
\frac{\partial H^*}{\partial y^*}
=
\frac{\partial h_{s}^*}{\partial y^*}
+
u^*
\frac{\partial u^*}{\partial y^*}
=
\frac{\partial }{\partial y^*}
\left(
    \sum_ih_{si}Y_i
\right)
+
u^*
\frac{\partial u^*}{\partial y^*}
=
\sum_i
\frac{\partial Y_i}{\partial y^*}
h_{si}
+
\sum_i
Y_i
\frac{\partial h_{si} }{\partial y^*}
+
u^*
\frac{\partial u^*}{\partial y^*}
\end{equation*} 

Then 

\begin{equation*} 
\frac{\partial H^*}{\partial y^*}
-
u^*
\frac{\partial u^*}{\partial y^*}
=
\sum_i
Y_i^*
\frac{\partial h_{si}^* }{\partial y^*}
+
\sum_i
\frac{\partial Y_i^*}{\partial y^*}
h_{si}^*
\end{equation*} 


\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left[
    \frac{k^*}{c_p^*}
    \left(
        \frac{\partial H^*}{\partial y^*}
        -
        u^*
        \frac{\partial u^*}{\partial y^*}
    \right)
\right]
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 

Joined the velocity gradient term  

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left[
    \frac{k^*}{c_p^*}
    \frac{\partial H^*}{\partial y^*}
    +
    \frac{k^*}{c_p^*}
    \left(
        \frac{\mu^*{c_p^*}}{k^*}
        -
        1 
    \right)
    u^*
    \frac{\partial u^*}{\partial y^*}
\right]
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 

An interesting term  appear in the velocity gradient.  
Remember that \\$Pr=\mu^*c_p^*/k^*$. If $Pr=1$ 
the heat diffusivity and the momentum diffusivity are 
the same, and this term vanish form the energy equation. 

Similarly another interesting term appear. 
Beginning with the total enthalpy equation: 

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left(
    \mu
    u^*
    \frac{\partial u^*}{\partial y^*}
\right)
+
\frac{\partial}{\partial y^*}
\left[
    \frac{k^*}{c_p^*}
    \left(
        \sum_i
        \frac{\partial Y_i}{\partial y^*}
        h_{si}
        +
        \sum_i
        Y_i
        \frac{\partial h_{si} }{\partial y^*}
    \right)
\right]
-
\frac{\partial}{\partial y^*}
\left(
        \sum_i
        h_{si}^*
        \rho^* 
        D_i^*
        \frac{\partial Y_i^*}{\partial y^*}
\right)
\end{split} 
\end{equation*} 

Splitting  the gradient of the heat conduction in term of 
sensible  enthalpy of species, join with the  diffusion of energy by species   
and put the work of viscous diffusion 

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left[
    \frac{k^*}{c_p^*} 
    \left(
    \sum_i
    Y_i
    \frac{\partial h_{si} }{\partial y^*}
    +
    \frac{\mu c_p^*}{k^*}   
    u^*
    \frac{\partial u^*}{\partial y^*}
    \right)
\right]
-
\frac{\partial}{\partial y^*}
    \left[
        \sum_i
        h_{si}^*
        \left(
            \frac{k^*}{c_p^*} 
            -
            \rho^* 
            D_i^*
        \right)
        \frac{\partial Y^*_i}{\partial y^*}
    \right]
\end{split} 
\end{equation*} 

The derivate of the Total enphalpy is 

\begin{equation*} 
\frac{\partial H^*}{\partial y^*}
-
\sum_i
\frac{\partial Y_i}{\partial y^*}
h_{si}
-
u^*
\frac{\partial u^*}{\partial y^*}
=
\sum_i
Y_i
\frac{\partial h_{si} }{\partial y^*}
\end{equation*} 

Therefore

\begin{equation*} 
\begin{split} 
 \frac{\partial}{\partial y^*}
\left[
    \frac{k^*}{c_p^*}
    \left(
    \frac{\partial H^*}{\partial y^*}
    -
    \sum_i
    \frac{\partial Y_i}{\partial y^*}
    h_{si}
    +
    \left(
    \frac{\mu c_p^*}{k^*}   
    -
    1
    \right)
    u^*
    \frac{\partial u^*}{\partial y^*}
    \right)
\right]
-
\frac{\partial}{\partial y^*}
    \left[
        \sum_i
        h_{si}^*
        \left(
            \frac{k^*}{c_p^*} 
            -
            \rho^* 
            D_i^*
        \right)
        \frac{\partial Y^*_i}{\partial y^*}
    \right]
\end{split} 
\end{equation*} 

Defining 
$Le_i=k^*/(D_i^*c_p^* \rho^*)$

\begin{equation*} 
\begin{split} 
\frac{\partial}{\partial y^*}
\left[
    \frac{ k^*}{c_p^*} 
    \left(
    \frac{\partial H^*}{\partial y^*}
    -
    \sum_i
    \frac{\partial Y_i}{\partial y^*}
    h_{si}
    +
    \left(
    Pr
    -
    1
    \right)
    u^*
    \frac{\partial u^*}{\partial y^*}
    \right)
\right]
-
\frac{\partial}{\partial y^*}
    \left[
    \frac{k^*}{c_p^*} 
        \sum_i
        h_{si}^*
        \left(
            1
            -
            \frac{1}{Le_1} 
        \right)
        \frac{\partial Y^*_i}{\partial y^*}
    \right]
\end{split} 
\end{equation*} 

If $Le_i=1$ and $Pr=1$, the total enthalpy equation can be simplified 

\begin{equation} 
\begin{split} 
\rho^* u^* \frac{\partial H^*}{\partial x^*}
+
\rho^* v^* \frac{\partial H^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left[
    \frac{ k^*}{c_p^*} 
    \left(
        \frac{\partial H^*}{\partial y^*}
        -
        \sum_i
        \frac{\partial Y_i}{\partial y^*}
        h_{si}
    \right)
\right]
\end{split} 
\end{equation} 

An important simplification is obtained 
assuming that  \\ $c_p=c_{pi}=constant$ which is the 
same that have equal thermal enthalpy for each specie.

\begin{equation} 
\begin{split} 
\rho^* u^* \frac{\partial H^*}{\partial x^*}
+
\rho^* v^* \frac{\partial H^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left[
        \frac{ k^*}{c_p^*} 
        \frac{\partial H^*}{\partial y^*}
        -
        \frac{ k^*}{c_p^*} 
        \left(
            \sum_i
            \frac{\partial Y_i h_{si}}{\partial y^*}
            -
            \sum_i
            Y_i
            \frac{\partial  h_{si}}{\partial y^*}
        \right)
\right]
\end{split} 
\end{equation} 

\begin{equation} 
\begin{split} 
\rho^* u^* \frac{\partial H^*}{\partial x^*}
+
\rho^* v^* \frac{\partial H^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left[
        \frac{ k^*}{c_p^*} 
        \frac{\partial H^*}{\partial y^*}
        -
        \frac{ k^*}{c_p^*} 
        \left(
            \frac{\partial  h_s}{\partial y^*}
            -
            \sum_i
            Y_i
            \frac{\partial  h_{si}}{\partial y^*}
        \right)
\right]
\end{split} 
\end{equation} 


Then, 

\begin{equation} 
\begin{split} 
\rho^* u^* \frac{\partial H^*}{\partial x^*}
+
\rho^* v^* \frac{\partial H^*}{\partial y^*}
=
\frac{\partial}{\partial y^*}
\left[
    \frac{ k^*}{c_p^*} 
    \left(
        \frac{\partial H^*}{\partial y^*}
   \right)
\right]
=
\frac{\partial}{\partial y^*}
\left[
    \frac{ \mu}{Pr} 
    \left(
        \frac{\partial H^*}{\partial y^*}
   \right)
\right]
=
\frac{\partial}{\partial y^*}
\left[
    \mu 
    \left(
        \frac{\partial H^*}{\partial y^*}
   \right)
\right]
\end{split} 
\end{equation} 

Finally, to complete the system the ideal gas law 
is used  

\begin{equation} 
p^*=\rho^*R^*T^*
\end{equation} 


%%%%
%book
%
%\begin{equation*} 
%\begin{split} 
%\frac{\partial}{\partial y^*}
%\left[
%    \frac{\mu*}{Pr}
%    \left(
%        \frac{\partial H^*}{\partial y^*}
%        +
%        (Pr-1)
%        u^*
%        \frac{\partial u^*}{\partial y^*}
%    \right)
%\right]
%\\
%+
%\frac{\partial}{\partial y^*}
%\left(
%        \sum_i^N
%        (Le^{-1}-1)
%        h_{si}^*
%        \frac{\partial Y_i^*}{\partial y^*}
%\right)
%\end{split} 
%\end{equation*} 
%
%\begin{equation*} 
%\begin{split} 
%\frac{\partial}{\partial y^*}
%\left[
%    \frac{\mu*k^*}{c_p^*\mu^*}
%    \left(
%        \frac{\partial H^*}{\partial y^*}
%        +
%        (\frac{k^*}{c_p^*\mu^*}
%        -1
%        )
%        u^*
%        \frac{\partial u^*}{\partial y^*}
%    \right)
%\right]
%\\
%+
%\frac{\partial}{\partial y^*}
%\left(
%        \sum_i^N
%        \frac{(\rho^*D_i^*c_p^*-1)}{k^*}
%        h_{si}^*
%        \frac{\partial Y_i^*}{\partial y^*}
%\right)
%\end{split} 
%\end{equation*} 

